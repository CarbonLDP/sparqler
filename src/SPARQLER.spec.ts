...
describe( "Module SPARQLER", () => {
	...
	describe( "Class SPARQLER", ():void => {
		...
		it( "SPARQLER.toCompactString()", ():void => {
			let sparqler:SPARQLER = new SPARQLER();

			// Return an empty string
			let query:string = sparqler.toCompactString();
			expect( query ).toEqual( jasmine.any( String ) );
			expect( query ).toBe( "" );
			...
			sparqler = new SPARQLER();
			sparqler[ "_base" ] = "http://example.com/";
			query = sparqler.toCompactString();
			expect( query ).toContain( "BASE<http://example.com/>" );

			// Prefix query construction
			sparqler = new SPARQLER();
			sparqler[ "_prefixes" ] = new Map<string, PrefixInfo>();
			sparqler[ "_prefixes" ].set( "ex", { iri: "http://example.com/ns#", used: false } );
			sparqler[ "_prefixes" ].set( "another", { iri: "http://another-example.com/ns#", used: false } );
			query = sparqler.toCompactString();
			// Does not add any prefix if not used ( just un compact string )
			expect( query ).toBe( "" );
			...
			query = sparqler.toCompactString();
			// Add only used prefix
			expect( query ).toContain( "PREFIX another:<http://another-example.com/ns#>" );


			let tokenSpy = spyOn( MockToken.prototype, "getTokenValue" ).and.callThrough();

			// Tests addition of `selects`
			sparqler = new SPARQLER();
			sparqler[ "_selects" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `from`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_from" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `where`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_where" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `group`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_group" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `having`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_having" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `order`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_order" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `limit`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_limit" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );

			// Tests addition of `offset`
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_offset" ] = [ new MockToken( "a" ), new MockToken( "b" ) ];
			query = sparqler.toCompactString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.COMPACT, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );
		...
		it( "SPARQLER.toPrettyString()", ():void => {
			let sparqler:SPARQLER = new SPARQLER();

			// Return an empty string
			let query:string = sparqler.toPrettyString();
			expect( query ).toEqual( jasmine.any( String ) );
			expect( query ).toBe( "" );
			...
			sparqler[ "_prefixes" ].set( "another", { iri: "http://another-example.com/ns#", used: true } );
			query = sparqler.toPrettyString();
			// Add every prefix even if not used
			expect( query ).toContain( "PREFIX ex: <http://example.com/ns#>" );
			expect( query ).toContain( "PREFIX another: <http://another-example.com/ns#>" );
			...
			query = sparqler.toPrettyString();
			expect( tokenSpy ).toHaveBeenCalledTimes( 2 );
			expect( tokenSpy ).toHaveBeenCalledWith( TokenFormat.PRETTY, jasmine.any( Token ) );
			expect( query ).toContain( "ab" );
			...
			// Tests the indentation
			tokenSpy.calls.reset();
			sparqler = new SPARQLER();
			sparqler[ "_selects" ] = [
				OPEN_MULTI_BLOCK, OPEN_MULTI_BLOCK, OPEN_MULTI_BLOCK,
				new MockToken( "a" ), EMPTY_SEPARATOR,
				CLOSE_MULTI_BLOCK, CLOSE_MULTI_BLOCK, CLOSE_MULTI_BLOCK,
			];
			query = sparqler.toPrettyString();
