...
describe( "whereDecorator", ():void => {
	...
	it( "should return a GroupClause & GraphPattern object", ():void => {
		const container:Container<GraphPattern> = new Container( graphPatternDecorator );
		const whereClause:WhereClause<GraphPattern> = whereDecorator( container, {} );

		const groupGraphClause:GroupClause<GraphPattern> & GraphPattern = whereClause.where( () => [] );
